...
function EditModal({ titolo, onSave, onClose, token }) {
  ...
  const handleSave = async () => {
    ...
    editableFields.forEach(k => {
      ...
      if (valForm !== valOrig) { payload[k] = valForm; }
    });
...
function ModuloFineGiro({ titoli, prenotato, canali, token, ruolo, spalmatura }) {
  ...
  const [filterRegioni, setFilterRegioni] = useState([]); // NUOVO: filtro regione
  const [search, setSearch] = useState("");
  ...
  const [clienteRegioneMap, setClienteRegioneMap] = useState({}); // NUOVO: codice_cliente → regione

  const resetFiltri = () => {
    setGiroLabelSel(null); setExtraSel(null); setCedolaSel("tutti");
    setFilterEditori([]); setFilterAccount("tutti"); setFilterCanale("tutti");
    setFilterRegioni([]); setSearch(""); setClienteSel(null);
  };

  // Fetch lista clienti per dropdown
  useEffect(() => {
    if (!token) return;
  ...
  // NUOVO: Fetch tabella clienti per mappa codice→regione
  useEffect(() => {
    if (!token) return;
  ...
  // Fetch prenotato del cliente selezionato
  useEffect(() => {
  ...
  // Fetch prenotato AURORA per editing manuale
  useEffect(() => {
    if (!token) return;
  ...
  // NUOVO: lista regioni disponibili dalla mappa
  ...
  // NUOVO: prenotatoCliente filtrato per regione (attivo solo con clienteSel)
  const prenotatoClienteFiltrato = useMemo(() => {
    if (filterRegioni.length === 0 || !clienteSel) return prenotatoCliente;
    // Nella vista cliente il codice_cliente è sempre clienteSel,
    // quindi filtriamo in base alla regione del cliente stesso
    const regioneCliente = clienteRegioneMap[clienteSel];
    if (!regioneCliente) return prenotatoCliente;
    return filterRegioni.includes(regioneCliente) ? prenotatoCliente : [];
  }, [prenotatoCliente, filterRegioni, clienteRegioneMap, clienteSel]);

  // NUOVO: prenotato aggregato filtrato per regione
  // Il prenotato aggregato non ha codice_cliente per riga, ma possiamo filtrare
  // tramite la tabella prenotato_clienti che li ha. Usiamo un Set di titolo_id
  // che hanno prenotato da clienti nelle regioni selezionate.
  // Per semplicità e performance, questo filtro è applicato a livello di righe/totali
  // nel useMemo righe qui sotto.

  const byCanaleCliente = useMemo(() => {
    if (!clienteSel || prenotatoClienteFiltrato.length === 0) return null;
    const map = {};
    prenotatoClienteFiltrato.forEach(p => { const c = canali.find(c => c.id === p.canale_id); if (c) map[c.codice] = (map[c.codice] || 0) + p.quantita; });
    return map;
  }, [clienteSel, prenotatoClienteFiltrato, canali]);
  ...
  const prenotatoClienteByTitolo = useMemo(() => {
    if (!clienteSel || prenotatoClienteFiltrato.length === 0) return null;
    const map = {};
    prenotatoClienteFiltrato.forEach(p => {
  ...
  const righe = useMemo(() => titoliSel.map(t => {
    ...
    if (clienteSel && prenotatoClienteByTitolo) {
      // Vista cliente: usa prenotatoClienteFiltrato (già filtrato per regione se attivo)
      const byCanale = {};
      prenotatoClienteFiltrato
        .filter(p => p.titolo_id === t.id)
        .forEach(p => { const c = canali.find(c => c.id === p.canale_id); if (c) byCanale[c.codice] = (byCanale[c.codice] || 0) + p.quantita; });
    ...
    // Vista aggregata: se filterRegioni attivo, filtra il prenotato per regione
    // usando la join con prenotato_clienti (disponibile in clientiList + clienteRegioneMap)
    let prenFiltrato = prenotato.filter(p => p.titolo_id === t.id);

    const byCanale = {};
    prenFiltrato.forEach(p => { const c = canali.find(c => c.id === p.canale_id); if (c) byCanale[c.codice] = p.quantita; });
    const dirStampatore = ruolo !== "agente" ? (auroraEdit[t.id] || 0) : 0;
    ...
    const totPrenBase = prenFiltrato.reduce((s, p) => s + p.quantita, 0);
    return { titolo: t, totPren: totPrenBase, byCanale, byCanaleObj, dirStampatore };
  }), [titoliSel, prenotato, canali, auroraEdit, ruolo, clienteSel, prenotatoClienteByTitolo, prenotatoClienteFiltrato, getObjCanalePerTitolo, filterRegioni, clienteRegioneMap]);
  ...
  return (
    <div style={{ display: "flex", flexDirection: "column", height: "100%", overflow: "hidden" }}>
      {/* Toolbar filtri */}
      <div style={{ padding: "12px 20px", borderBottom: `1px solid ${T.border}`, display: "flex", gap: 10, alignItems: "center", flexWrap: "wrap" }}>
        <select style={css.input} value={giroLabelSel || ""} onChange={e => { setGiroLabelSel(e.target.value || null); setExtraSel(null); setCedolaSel("tutti"); setFilterEditori([]); setFilterAccount("tutti"); setFilterCanale("tutti"); setFilterRegioni([]); }}>
      ...
      {clienteSel && byCanaleCliente && (
        <div style={{ padding: "10px 20px", borderBottom: `1px solid ${T.border}`, background: T.accent + "11" }}>
          <div style={{ color: T.accent, fontWeight: "700", fontSize: "11px", marginBottom: 6 }}>
            ...
            {filterRegioni.length > 0 && (
              <span style={{ color: T.textMid, fontWeight: "400", marginLeft: 12 }}>
                · Regioni: {filterRegioni.join(", ")}
              </span>
            )}
          </div>
